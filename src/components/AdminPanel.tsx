...
export default function AdminPanel() {
  const { blogs, addBlog, updateBlog, deleteBlog, isAdmin, loading } = useBlogs();
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-16">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-6 mb-12 border-b-4 border-black pb-8">
        <div>
          ...
          <p className="text-[10px] uppercase font-bold tracking-[0.4em] text-editorial-text opacity-50 mt-2">RollFetch Newsroom</p>
        </div>
        <button 
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-10">
            <div className="space-y-8">
              ...
              <div>
                <label className="block text-[10px] font-black uppercase tracking-widest mb-2 opacity-60">Feature Image URL</label>
                <input 
                  type="text" 
                  value={currentBlog.imageUrl}
                  onChange={e => setCurrentBlog({...currentBlog, imageUrl: e.target.value})}
                  className="w-full bg-editorial-aside border-2 border-primary/10 p-4 text-xs font-bold focus:border-primary outline-none transition-all text-editorial-text"
                />
              </div>
